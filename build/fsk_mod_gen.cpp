...
impl mydsp {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("filename", r"fsk_mod.dsp");
	...
	pub fn build_user_interface_static(ui_interface: &mut dyn UI<FaustFloat>) {
		ui_interface.open_vertical_box("fsk_mod");
		ui_interface.add_horizontal_slider("fsk/carrier_freq", ParamIndex(0), 0.1, 0.001, 0.5, 0.0001);
		ui_interface.add_horizontal_slider("fsk/deviation_hz", ParamIndex(1), 4.8e+03, 1e+02, 5e+04, 1e+01);
		ui_interface.add_horizontal_slider("fsk/output_gain", ParamIndex(2), 0.9, 0.0, 1.0, 0.001);
	...
	pub fn compute(
		...
		for ((input0, output0), output1) in zipped_iterators {
			...
			let mut fTemp0: F32 = (if i32::wrapping_sub(1, self.iVec0[1]) != 0 {0.0} else {self.fRec0[1] + self.fConst3 * (self.fRec2[0] + self.fConst3 * *input0 * self.fRec1[0])});
			self.fRec0[0] = fTemp0 - F32::floor(fTemp0);
			let mut fTemp1: F32 = 6.2831855 * self.fRec0[0];
			*output0 = fSlow2 * F32::cos(fTemp1);
			*output1 = fSlow2 * F32::sin(fTemp1);
