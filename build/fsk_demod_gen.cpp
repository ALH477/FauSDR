...
impl mydsp {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("filename", r"fsk_demod.dsp");
	...
	pub fn instance_constants(&mut self, sample_rate: i32) {
		...
		self.fConst4 = 3.1415927 / mydsp_faustpower2_f(self.fConst0);
		self.fConst5 = 1.25 / self.fConst0;
	}
	...
	pub fn build_user_interface_static(ui_interface: &mut dyn UI<FaustFloat>) {
		ui_interface.open_vertical_box("fsk_demod");
		ui_interface.add_horizontal_slider("fsk_demod/deviation_hz", ParamIndex(0), 4.8e+03, 1e+02, 5e+04, 1e+01);
		ui_interface.add_horizontal_slider("fsk_demod/lim_eps", ParamIndex(1), 1e-06, 1e-09, 0.01, 1e-09);
		ui_interface.add_horizontal_slider("fsk_demod/mode", ParamIndex(2), 0.0, 0.0, 1.0, 1.0);
		ui_interface.add_horizontal_slider("fsk_demod/output_gain", ParamIndex(3), 1.0, 0.0, 8.0, 0.01);
		ui_interface.add_horizontal_slider("fsk_demod/symbol_rate", ParamIndex(4), 9.6e+03, 3e+02, 1.152e+05, 1.0);
	...
	pub fn compute(
		...
		let mut iSlow1: i32 = (self.fHslider1 > 0.5) as i32;
		...
		for ((input0, input1), output0) in zipped_iterators {
			...
			let mut fTemp3: F32 = *input1;
			let mut fTemp4: F32 = *input0;
			let mut fTemp5: F32 = fSlow2 + F32::sqrt(mydsp_faustpower2_f(fTemp4) + mydsp_faustpower2_f(fTemp3));
			let mut fTemp6: F32 = fTemp4 / fTemp5;
			self.fVec0[0] = fTemp6;
			let mut fTemp7: F32 = fTemp3 / fTemp5;
			self.fVec1[0] = fTemp7;
			let mut fTemp8: F32 = fTemp1 + 1.0;
			self.fRec3[0] = fSlow3 + self.fConst2 * self.fRec3[1];
			let mut fTemp9: F32 = self.fConst4 * self.fRec3[0];
			let mut fTemp10: F32 = F32::tan(-fTemp9);
			let mut fTemp11: F32 = 1.0 / fTemp10;
			let mut fTemp12: F32 = (fTemp11 - self.fConst5) / fTemp10 + 1.0;
			let mut fTemp13: F32 = mydsp_faustpower2_f(fTemp10);
			self.fRec4[0] = fTemp6 - (self.fRec4[2] * ((self.fConst5 + fTemp11) / fTemp10 + 1.0) + 2.0 * self.fRec4[1] * (1.0 - 1.0 / fTemp13)) / fTemp12;
			let mut fTemp14: F32 = mydsp_faustpower2_f(self.fRec4[0] - self.fRec4[2]) / (fTemp13 * mydsp_faustpower2_f(fTemp12));
			self.fVec2[0] = fTemp14;
			let mut fTemp15: F32 = 1.0 - fTemp1;
			self.fRec2[0] = -((fTemp15 * self.fRec2[1] - (fTemp14 + self.fVec2[1])) / fTemp8);
			let mut fTemp16: F32 = F32::tan(fTemp9);
			let mut fTemp17: F32 = 1.0 / fTemp16;
			let mut fTemp18: F32 = (self.fConst5 + fTemp17) / fTemp16 + 1.0;
			let mut fTemp19: F32 = mydsp_faustpower2_f(fTemp16);
			self.fRec6[0] = fTemp6 - (self.fRec6[2] * ((fTemp17 - self.fConst5) / fTemp16 + 1.0) + 2.0 * self.fRec6[1] * (1.0 - 1.0 / fTemp19)) / fTemp18;
			let mut fTemp20: F32 = mydsp_faustpower2_f(self.fRec6[0] - self.fRec6[2]) / (fTemp19 * mydsp_faustpower2_f(fTemp18));
			self.fVec3[0] = fTemp20;
			self.fRec5[0] = -((self.fRec5[1] * fTemp15 - (fTemp20 + self.fVec3[1])) / fTemp8);
			self.fRec1[0] = (if iSlow1 != 0 {self.fRec5[0] - self.fRec2[0]} else {(fTemp4 * (fTemp7 - self.fVec1[1]) - fTemp3 * (fTemp6 - self.fVec0[1])) / fTemp5}) - (self.fRec1[2] * ((fTemp1 + -1.4142135) / fTemp0 + 1.0) + 2.0 * self.fRec1[1] * (1.0 - 1.0 / mydsp_faustpower2_f(fTemp0))) / fTemp2;
			*output0 = fSlow4 * ((self.fRec1[2] + self.fRec1[0] + 2.0 * self.fRec1[1]) / fTemp2);
